...
void test_pipeline() {
  ...
  for (int i = 0; i < SPECTRUM_WIDTH * SPECTRUM_HEIGHT; i++) {
    // ESP_LOGI(TAG_INF, "Feature %d: Expected: %f, Actual: %f", i, test_x[i],
    // x[i]);
  ...
  for (int i = 0; i < SPECTRUM_WIDTH * SPECTRUM_HEIGHT; i++) {
    // ESP_LOGI(TAG_INF, "Quantized Feature %d: Expected: %d, Actual: %d", i,
    // test_xq[i], xq[i]);
  ...
  for (int i = 0; i < NUM_CLASSES; i++) {
    ESP_LOGI(TAG_INF, "Class %d: Expected: %f, Actual: %f", i,
             test_prediction[i], prediction[i]);
    ASSERT(fabs(test_prediction[i] - prediction[i]) < 5e-2);
  }
