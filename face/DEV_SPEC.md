...
## 10. Implementation Status (Updated 2026-04-22)

This section documents what was actually implemented vs. the original design, including deviations and rationale.

### 10.1 Design Deviations

| Dimension | Original Design | Actual Implementation | Rationale |
|:----------|:----------------|:----------------------|:----------|
| Input resolution | 64×64 grayscale | **48×48 grayscale** | 48×48 achieved 98.7% test accuracy, sufficient for 3-class task. Saves SRAM (~2.3 KB vs ~4 KB input). DEV_SPEC listed 48×48 as fallback. |
| Output formulation | 4-class softmax (A/B/C/Unknown) | **3-class softmax + threshold reject** | 4-class training caused significant accuracy drop because Unknown class data is too heterogeneous. 3-class with confidence threshold is more practical. |
| Unknown rejection | Softmax confidence only | **Center Loss training + Softmax threshold + Multi-frame voting** | Plain softmax gives high confidence even on non-faces. Center Loss improves softmax calibration (unknown rejection: 25% → 79%). Multi-frame voting further filters unstable predictions. |
| Confidence threshold | 0.80 | **0.95** | Higher threshold needed because model is overconfident on non-face inputs. 0.95 filters most non-face scenes while maintaining recognition for team members. |
| Normalization | z-score: (x - MEAN) / STD | **Simple scaling: x / 255.0** | z-score not implemented in training pipeline. Simple normalization achieved sufficient accuracy. |

### 10.2 ML Pipeline (Kirsi)

**Training scripts** (in `ml_share/ml_scripts/`):

| Script | Purpose | Status |
|:-------|:--------|:------:|
| `1build_image_index.py` | Build image index from raw data | Done |
| `4build_image_manifest.py` | Expand range manifest to per-image manifest | Done |
| `5split_holdout_val_test.py` | Split holdout into val/test | Done |
| `7_1train_gray48_cnn.py` | Baseline CNN (overfitting) | Done |
| `7_2train_gray48_cnn_aug.py` | CNN + data augmentation (test_acc=98.7%) | Done |
| `7_3train_center_loss.py` | CNN + center loss (improved unknown rejection) | Done |
| `9_eval_embedding_distance.py` | Embedding distance evaluation | Done |
| `10quantize_gray48_cnn_center_int8.py` | INT8 quantization | Done |
| `11eval_gray48_cnn_center_int8.py` | Quantized model evaluation | Done |
| `12export_tflite_to_c_array.py` | Export to model_data.c/h | Done |

**Model evolution**:

| Model | Test Acc | Unknown Rejection (softmax) | Notes |
|:------|:--------:|:---------------------------:|:------|
| gray48_cnn (baseline) | 53.3% | - | Severe overfitting |
| gray48_cnn_aug | 98.7% | 25% | Data augmentation solved overfitting |
| gray48_cnn_center (λ=0.01) | 98.7% | 79% | Center loss improved softmax calibration |
| gray48_cnn_center_int8 | ~97% | ~79% | INT8 quantization, deployed to ESP32 |

**Center Loss hyperparameter search** (7 configurations tested in parallel):

Best: λ=0.01, center_lr=0.5 — maintains 98.7% classification accuracy while maximizing embedding compactness (nearest-centroid accuracy: 63.6% → 97.4%).

### 10.3 ESP32 Firmware (Linxuan)

**File structure** (in `face/esp32/main/`):

| File | Source | Description |
|:-----|:-------|:------------|
| `main.cpp` | New | Main loop: camera → preprocess → inference → vote → LED |
| `camera.h/.cpp` | Copied from `camera/esp32/` | OV2640 driver, 320×240 RGB565 |
| `preprocess.h/.cpp` | New | RGB565 → 48×48 grayscale: center-crop 240×240, 5×5 area average, /255.0 |
| `inference.h/.cpp` | Adapted from `keywords/esp32/` | TFLite Micro wrapper, INT8 quantize/dequantize |
| `model_data.h/.c` | Generated by `12export_tflite_to_c_array.py` | INT8 quantized CNN, 30 KB |
| `CMakeLists.txt` | New | Build configuration |
| `idf_component.yml` | Adapted from `keywords/esp32/` | Dependencies: esp-tflite-micro, esp32-camera |

**Key adaptations from keywords project**:

| Component | Keywords (Audio) | Face (Image) |
|:----------|:-----------------|:-------------|
| Input | Audio frames → FFT spectrogram | Camera frame → grayscale resize |
| Model variable | `model_binary` | `g_model_data` |
| Input size | `SPECTRUM_WIDTH * HEIGHT` | `48 * 48 = 2304` |
| Op resolver | 9 ops (Conv1D via Reshape) | 9 ops (same set, Conv2D native) |
| Tensor arena | 30 KB | 60 KB |
| Output | 3 classes (Other/Yes/No) | 3 classes (person_a/b/c) |

**Multi-frame voting**:
- Window size: 5 frames
- Threshold: 3/5 frames must agree
- Purpose: filters unstable predictions on non-face inputs (hand, desk, wall)
- Combined with confidence threshold 0.95

**Performance measurements**:

| Metric | Target | Achieved |
|:-------|:------:|:--------:|
| Test accuracy | ≥ 90% | **98.7%** (float), ~97% (INT8) |
| Inference latency | ≤ 200 ms | **~220 ms** per frame |
| Frame rate | ≥ 1 FPS | **~4.5 FPS** |
| Model binary size | ≤ 50 KB | **30 KB** (INT8 TFLite) |
| Firmware binary size | ≤ 1 MB | **402 KB** (62% partition free) |

**sdkconfig changes**:
- `CONFIG_SPIRAM=y` — Enable PSRAM for camera frame buffer (153 KB)
- `CONFIG_SPIRAM_MODE_OCT=y` — Octal SPI mode (required by XIAO ESP32S3 Sense)

### 10.4 Implementation Steps — Actual Progress

| # | Step | Status | Notes |
|:-:|:-----|:------:|:------|
| 1 | Flash `camera/esp32` unchanged | Done | Board streams frames |
| 2 | Create `face/python/collect.py` | Done | Data collection tool |
| 3 | Collect images (3 known + unknown) | Done | 258+258+259+259 images |
| 4 | Build data pipeline (index, manifest, split) | Done | Scripts 1, 4, 5 |
| 5 | Train baseline CNN | Done | Overfitting (53% test acc) |
| 6 | Train CNN + augmentation | Done | 98.7% test acc |
| 7 | Evaluate unknown rejection | Done | Softmax: 25%, embedding distance analysis |
| 8 | Train CNN + center loss | Done | λ=0.01, unknown rejection: 79% |
| 9 | INT8 quantization + C export | Done | 30 KB model_data.c |
| 10 | Set up `face/esp32/` project | Done | Compiles with ESP-IDF 6.1 |
| 11 | Write `preprocess.cpp` | Done | RGB565 → 48×48 grayscale |
| 12 | Adapt `inference.cpp` | Done | TFLite Micro with 9 ops, 60 KB arena |
| 13 | Write `main.cpp` | Done | Camera → preprocess → inference → vote → LED |
| 14 | Build, flash, monitor | Done | LED responds, ~4.5 FPS |
| 15 | Add multi-frame voting | Done | 5-frame window, 3/5 threshold |
| 16 | Create `face/python/display.py` | TODO | Serial frame + result overlay |
| 17 | [Bonus] On-device test alignment | TODO | test_case.h verification |

### 10.5 Team Member Contributions

| Member | Responsibilities |
|:-------|:----------------|
| Kirsi | ML pipeline: data processing, model training (baseline → augmentation → center loss), INT8 quantization, TFLite export |
| Linxuan | ESP32 deployment: firmware development (preprocess, inference, main loop, multi-frame voting), center loss training script, embedding distance evaluation, hardware-software integration |
| TODO | TODO |

*(To be completed before report submission.)*
